...
- Using a similar process you used to schedule the main part of the script, you can get a monthly summary of your spending and income. Just go through the same process as above, but select the `sendMonthlySummaryOnFirstDayOfMonth` function instead of `getTransactionsForDay`. You should choose a monthly trigger, on the first day of each month. Select a time a little further from midnight to ensure the date logic works correctly (I chose noon). Then every month on the 1st, you will receive an email detailing your spending and income for the previous month
